...
import type { AnalyticsData } from '@/features/performance/services/performance';
...
export default function PerformanceMetrics({ analytics, loading }: PerformanceMetricsProps) {
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: `${(analytics.promedio_puntajes ?? 0).toFixed(1)}%`,
      icon: TrendingUp,
      color: 'bg-green-500',
    ...
    },
    {
      ...
      value: `${(analytics.tasa_aprobacion ?? 0).toFixed(1)}%`,
      icon: CheckCircle,
    ...
    },
    {
      ...
      value: `${(analytics.tasa_reprobacion ?? 0).toFixed(1)}%`,
      icon: XCircle,
      color: 'bg-red-500',
